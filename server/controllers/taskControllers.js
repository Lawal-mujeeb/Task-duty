...
export const createTask = tryCatchFn(async (req, res, next) => {
  const task = await taskService.createTask(req.body, req.user._id, next);
...
export const getTasks = tryCatchFn(async (req, res, next) => {
  const tasks = await taskService.getTasks(req.user._id, req.query, next);
  if (!tasks) return;
...
export const getSingleTask = tryCatchFn(async (req, res, next) => {
  const task = await taskService.getSingleTask(req.params.id, req.user._id, next);
  if (!task) return;
...
export const updateTask = tryCatchFn(async (req, res, next) => {
  const updatedTask = await taskService.updateTask(req.params.id, req.body, req.user._id, next);
...
export const deleteTask = tryCatchFn(async (req, res, next) => {
  const result = await taskService.deleteTask(req.params.id, req.user._id, next);
  if (!result) return;
